...
const Alerts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const docRef = doc(db, 'HDBS', 'AlertText')
      const docSnap = await getDoc(docRef)

      if (docSnap.exists()) {
        setAlert(docSnap.data().text)
        console.log(alert)
      }
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.header}>
          <Text style={styles.category}>Alerts</Text>
          {/* <View style={styles.dateTimeContainer}>
              <Text style={styles.date}>{date + ", "}</Text>
              <Text style={styles.time}>{eventTime}</Text>
            </View> */}
        </View>
        <Text style={styles.eventName}>Alerts</Text>
        <Text style={styles.description}>{alert}</Text>
      </View>
    </SafeAreaView>
  );
}
