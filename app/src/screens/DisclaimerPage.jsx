...
const DisclaimerPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const docRef = doc(db, 'HDBS', 'Disclaimer')
      const docSnap = await getDoc(docRef)

      if (docSnap.exists()) {
        setDisclaimer(docSnap.data().text)
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Image source={require('../assets/Alertbg.png')} style={styles.image}></Image>
      <View style={styles.content}>
        <View style={styles.header}>
          <Text style={styles.category}>Disclaimer</Text>
          {/* <View style={styles.dateTimeContainer}>
              <Text style={styles.date}>{date + ", "}</Text>
              <Text style={styles.time}>{eventTime}</Text>
            </View> */}
        </View>
        <Text style={styles.eventName}>Disclaimer</Text>
        <Text style={styles.description}>{disclaimer}</Text>
      </View>
    </SafeAreaView>
  );
}
